...
public class CaesarAs2_6 {
    ...
    private static int printStringLength = 0;
...
/*
    ...
    public static void mainMethod(String[] inputs) {
        ...
        for (int j = printStringLength - 1; j >= 0; j--) {
            System.out.print(printStringArr[j] + " ");
        }
        System.out.println();
    }
    ...
    public static void aschiiShift(int i) {
        int shiftInt = Integer.parseInt(inputs[i]);
        String[] tempString = new String[head];
        int localHead = 0;
        for (int j = 0; j < head; j++) {
            if (!isInteger(String.valueOf(printStringArr[j].charAt(0))) && printStringArr[j].charAt(0) > 64 && printStringArr[j].charAt(0) < 91) {
                int cc = printStringArr[j].charAt(0);
                String temp = runner(chars, shiftInt, cc - 64);
                tempString[localHead] = temp;
                localHead++;
            } else if (printStringArr[j].charAt(0) != 32) {
                try {
                    tempString[localHead] = printStringArr[j];
                    localHead++;
                } catch (Exception e) {
                    e.printStackTrace();
                }
        ...
        for (int j = 0; j < head; j++) {
            printStringArr[j] = tempString[j];
        }
    }

  /*  static String runner(String[] A, int runTime, int initial) {
        String temp = "";
        for (int i = initial; i < runTime + initial; i++) {
            temp = A[((i - 1) % 25)];
        }
        return temp;
    }*/
