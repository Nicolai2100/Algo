...
*/
       // inputs = input.split(" ");
        ...
        printString = "";
        while (i > -1) {
            if (!input.substring(i,i+1).equalsIgnoreCase("^") && !isInteger(input.substring(i,i+1))) {
                printString += input.substring(i,i+1)+ " ";
                i--;
            } else if (isInteger(input.substring(i,i+1))) {
                printString = aschiiShift(i);
                i--;
                printString += input.substring(i+1,i+2) + " ";
    ...
    static String aschiiShift(int i) {
        int shiftInt = Integer.parseInt(input.substring(i,i+1));
        String tempString = "";
        for (int j = 0; j < printString.length(); j++) {
            System.out.println(printString.charAt(j));

            if (printString.charAt(j) > 64 && printString.charAt(j) < 91) {
                System.out.println(printString.charAt(j));
